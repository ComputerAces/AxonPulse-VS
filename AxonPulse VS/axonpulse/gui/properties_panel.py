...
class PropertiesPanel(QWidget):
    ...
    def load_node(self, node_or_widget):
        ...
        self.current_frame = None
        from axonpulse.gui.node_widget import NodeWidget
        ...
        if hasattr(logic_node, 'properties'):
            ...
            for key in prop_keys:
                ...
                if key.lower() == "compare_type" or key == "Compare Type" or dtype == DataType.COMPARE_TYPE or dtype == DataType.COMPARE:
                    self.add_dropdown_property_ui(key, value, ["<", "<=", ">", ">=", "==", "!="])
                elif dtype in [DataType.FLOW, DataType.PROVIDER_FLOW]:
                    # [STRICT HIDE] Do not show execution flow ports as vars
                    continue
                elif key.lower() == "value" and ("True/False" in logic_node.name or "Boolean" in logic_node.name):
                    self.add_dropdown_property_ui(key, value, ["<", "<=", ">", ">=", "==", "!="])
                elif key == "Trigger Type" or dtype == DataType.TRIGGER:
                    from axonpulse.core.types import TriggerType
        ...
        if self.current_widget and hasattr(self.current_widget, 'inputs'):
            try:
                ...
                for p in self.current_widget.inputs:
                    # Skip flow-class ports or flow-typed ports
                    if hasattr(p, 'port_class') and p.port_class == "flow":
                        continue
                    if hasattr(p, 'data_type') and p.data_type in [DataType.FLOW, DataType.PROVIDER_FLOW]:
                        continue
                ...
                if data_inputs:
                    ...
                    for port in data_inputs:
                        ...
                        # [NEW] Format Label with Type Abbreviation
                        abbr = DataType.get_abbreviation(dtype)
                        label_name = f"{port_name} ({abbr})" if abbr and abbr != "?" else port_name

                        if isinstance(val, bool) or dtype == DataType.BOOLEAN:
                            self.add_bool_property_ui(port_name, val if isinstance(val, bool) else False, label_name)
                        elif isinstance(val, (int, float)) or dtype in [DataType.NUMBER, DataType.INTEGER, DataType.FLOAT]:
                            self.add_number_property_ui(port_name, val if isinstance(val, (int, float)) else 0, label_name)
                        else:
                            self.add_string_property_ui(port_name, str(val) if val is not None else "", label_name)
        ...
        if self.current_widget and "Start" in getattr(self.current_widget, 'node_type', ''):
            data_outputs = []
            for p in self.current_widget.outputs:
                # Filter Flow
                is_flow = False
                if hasattr(p, 'port_class') and p.port_class == "flow": is_flow = True
                if hasattr(p, 'data_type') and p.data_type in [DataType.FLOW, DataType.PROVIDER_FLOW]: is_flow = True
                if p.name in ["Flow", "Error Flow"]: is_flow = True
                
                if not is_flow:
                    data_outputs.append(p)
            ...
            if data_outputs:
                ...
                for port in data_outputs:
                    ...
                    # [NEW] Format Label with Type Abbreviation
                    dtype = getattr(port, 'data_type', DataType.ANY)
                    abbr = DataType.get_abbreviation(dtype)
                    label_name = f"{out_name} ({abbr})" if abbr and abbr != "?" else out_name
                    
                    self.add_string_property_ui(out_name, str(val), label_name)
    ...
    def add_string_property_ui(self, key, value, label=None):
        ...
        self.form_layout.addRow(f"{label if label else key}:", container)
    ...
    def add_dropdown_property_ui(self, key, value, options, label=None):
        ...
        self.form_layout.addRow(f"{label if label else key}:", combo)
    
    def add_number_property_ui(self, key, value, label=None):
        ...
        self.form_layout.addRow(f"{label if label else key}:", spin)

    def add_bool_property_ui(self, key, value, label=None):
        from PyQt6.QtWidgets import QCheckBox
        cb = QCheckBox()
        cb.setChecked(value)
        cb.toggled.connect(lambda v, k=key: self.update_property(k, v))
        self.form_layout.addRow(f"{label if label else key}:", cb)
    ...
    def _open_frame_color_picker(self, frame):
        ...
        from axonpulse.gui.color_picker import ColorPickerDialog
