...
class FileOperationsMixin:
    ...
    def open_graph(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Open Graph", "", "AxonPulse Project (*.syp);;JSON Files (*.json)")
        if file_path:
    ...
    def save_graph(self, graph=None):
        ...
        if not graph.file_path:
            file_path, _ = QFileDialog.getSaveFileName(self, "Save Graph", "", "AxonPulse Project (*.syp)")
        ...
        try:
            ...
            self.setWindowTitle(f"AxonPulse VS - Architect - {display_name}")
    ...
    def save_graph_as(self):
        ...
        if not graph: return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Graph As", "", "AxonPulse Project (*.syp)")
        if not file_path: return
    ...
    def import_subgraph(self):
        graph = self.get_current_graph()
        if not graph: return
        file_path, _ = QFileDialog.getOpenFileName(self, "Import Subgraph", "", "AxonPulse Project (*.syp);;JSON Files (*.json)")
        if not file_path: return
        
        if hasattr(graph, 'canvas') and hasattr(graph.canvas, 'factory'):
            # Grab the center of the current view to drop the node
            # NodeCanvas inherits from QGraphicsView, so it IS the view.
            view_center = graph.canvas.mapToScene(graph.canvas.viewport().rect().center())
            
            # Delegate to factory which handles dynamic port extraction and registration
            new_node = graph.canvas.factory.create_subgraph_node(file_path, view_center)
            
            if new_node:
                graph.is_modified = True
                self.statusBar().showMessage(f"Imported Subgraph: {new_node.name}", 3000)
            else:
                QMessageBox.warning(self, "Import Error", "Failed to construct SubGraph node.")
        else:
            QMessageBox.warning(self, "Error", "Active graph does not support Canvas operations.")

    def import_to_library(self):
        try:
            if not hasattr(self, 'node_library'):
                QMessageBox.warning(self, "Error", "Node Library is currently unavailable.")
                return

            file_path, _ = QFileDialog.getOpenFileName(self, "Import Graph to Library", "", "AxonPulse Project (*.syp);;JSON Files (*.json)")
            if not file_path: return

            # Try to resolve the name to display
            ...
            self.node_library.add_favorite(file_path)
            
            # Refresh Registry and Library UI
            from axonpulse.core.loader import load_favorites_into_registry
            load_favorites_into_registry()
            self.node_library.populate_library()
            
            QMessageBox.information(self, "Import Successful", f"Added '{name}' to Node Library.")
        except Exception as e:
            import traceback
            QMessageBox.critical(self, "Crash in Import", f"Error:\n{str(e)}\n\n{traceback.format_exc()}")
    ...
    def broadcast_graph_modified(self):
        graph = self.get_current_graph()
        if not graph:
            self.setWindowTitle("AxonPulse VS - Architect")
            return
        ...
        self.setWindowTitle(f"AxonPulse VS - Architect - {project_name}{mod_mark}")
