...
from axonpulse.core.constants import IS_WINDOWS
...
@NodeRegistry.register("Toast", "UI/Toasts")
class ToastNode(SuperNode):
    """
    ...
    def __init__(self, node_id, name, bridge):
        ...
        self.properties["Title"] = "AxonPulse OS"
        self.properties["Message"] = "Notification clicked!"
        self.define_schema()
    ...
    def do_work(self, Title=None, Message=None, **kwargs):
        ...
        # Trim message to prevent Windows display failure (max 200 characters)
        if message_val and len(str(message_val)) > 200:
            message_val = str(message_val)[:197] + "..."

        ensure_toast()
        if IS_WINDOWS:
            return self._execute_windows(title_val, message_val)
        else:
            return self._execute_cross_platform(title_val, message_val)

    def _execute_windows(self, Title, Message):
        if not toast:
        ...
        try:
            # win11toast uses 'tag' to replace old notifications from the same source
            toast(Title, Message, on_click=on_click, app_id="AxonPulse OS", tag=self.node_id)
            # Windows toasts usually last ~10-15s
            if clicked_event.wait(timeout=15.0):
                # User clicked! Trigger both Done and OnClick
                self.bridge.set(f"{self.node_id}_ActivePorts", ["Flow", "OnClick"], self.name)
                return True
        except Exception as e:
            self.logger.error(f"Windows Toast Error: {e}")
            
        # If timeout or error, just continue standard Flow
        self.bridge.set(f"{self.node_id}_ActivePorts", ["Flow"], self.name)
        return True

    def _execute_cross_platform(self, Title, Message):
        ...
        async def notify():
            notifier = DesktopNotifier(app_name="AxonPulse OS")
            await notifier.send(
                title=Title,
                message=Message,
        ...
        try:
            import asyncio
            # We need a loop to run the async notifier
            ...
            self.logger.error(f"Cross-platform Notification Error: {e}")
