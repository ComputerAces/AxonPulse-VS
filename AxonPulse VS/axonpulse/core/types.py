...
class DataType(Enum):
    ...
    def get_abbreviation(cls, data_type):
        """Returns a short string representation of the data type (e.g. 'S', 'N', 'B')."""
        if isinstance(data_type, str):
            try: data_type = cls(data_type.lower())
            except: return "?"
...
class AxonPulseJSONEncoder(json.JSONEncoder):
    """Custom JSON encoder to handle AxonPulse Enums and potential non-serializable types."""
