...
class Wire(QGraphicsPathItem):
    ...
    def highlight_active(self, status="success"):
    ...
    def highlight_fade(self, status="success"):
        """Alias for pulse animation."""
        self.highlight_active(status=status)
    ...
    def paint(self, painter, option, widget):
        ...
        if alpha > 0:
            ...
            # [ERROR SPECIAL] Dual-Color Highlight
                ...
                # Check stored status
                status = getattr(self, "_active_status", "success")
                if status == "error":
                     pulse_col = QColor("#FF0000") # Bright Red for Error
                     self.pen.setWidth(self.width + 4) # Make it stand out more
