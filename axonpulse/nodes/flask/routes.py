...
class FlaskRouteNode(SuperNode):
    ...
    def __init__(self, node_id, name, bridge):
        ...
        self.properties["Override Rate Limit"] = ""
        self.define_schema()
    ...
    def handle_registration(self, Service_ID=None, Path=None, Method=None, **kwargs):
        ...
        try:
            ...
            # [DDOS] Apply per-route rate limit override if a limiter exists
            override_limit = self.properties.get("Override Rate Limit", "")
            if override_limit and service_id:
                limiter = self.bridge.get(f"{service_id}_limiter")
                if limiter:
                    try:
                        route_handler = limiter.limit(override_limit)(route_handler)
                        self.logger.info(f"Route {path} override limit: {override_limit}")
                    except Exception as e:
                        self.logger.warning(f"Failed to apply route limit override: {e}")
