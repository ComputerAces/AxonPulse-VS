    ...
    def subtract_images(self, **kwargs):
        ...
        def get_pil_image(val):
            if val is None or (isinstance(val, str) and not val.strip()): 
                # Silently skip null or empty strings
                return None
        ...
        for idx, (sort_key, val) in enumerate(sorted_items):
            # Skip empty values silently BEFORE calling get_pil_image or logging
            if val is None or (isinstance(val, str) and not val.strip()):
                continue
            ...
            if img:
                ...
                self.logger.warning(f"Failed to extract image from Item {idx} (Value: {val})")
